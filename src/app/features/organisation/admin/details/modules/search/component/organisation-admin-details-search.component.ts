...
import { selectClicksStatistics, selectCtrStatistics, selectImpressionsStatistics, selectPositionsStatistics } from '../state/organisation-admin-details-search.selectors';
...
export class OrganisationAdminDetailsSearchComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.activatedRoute.parent?.params.pipe(
      ...
      if (slug) {
        this.store.dispatch(OrganisationAdminDetailsSearchActions.init(
          slug,
          this.initPeriod,
          this.initInterval,
        ));
  ...
  public updatePeriod($organisation: Period): void {
    this.store.dispatch(OrganisationAdminDetailsSearchActions.updatePeriod($organisation));
  }

  public updateInterval($organisation: IntervalFilter): void {
    this.store.dispatch(OrganisationAdminDetailsSearchActions.updateInterval($organisation));
  }
