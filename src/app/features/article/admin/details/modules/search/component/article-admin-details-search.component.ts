...
import { selectClicksStatistics, selectCtrStatistics, selectImpressionsStatistics, selectPositionsStatistics } from '../state/article-admin-details-search.selectors';
...
export class ArticleAdminDetailsSearchComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.activatedRoute.parent?.params.pipe(
      ...
      if (slug) {
        this.store.dispatch(ArticleAdminDetailsSearchActions.init(
          slug,
          this.initPeriod,
          this.initInterval,
        ));
  ...
  public updatePeriod($article: Period): void {
    this.store.dispatch(ArticleAdminDetailsSearchActions.updatePeriod($article));
  }

  public updateInterval($article: IntervalFilter): void {
    this.store.dispatch(ArticleAdminDetailsSearchActions.updateInterval($article));
  }
